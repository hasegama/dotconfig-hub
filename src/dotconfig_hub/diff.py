...
class DiffViewer:
    ...
    def _display_unified_diff(self, source_path: Path, target_path: Path) -> None:
    ...
    def _display_side_by_side(self, source_path: Path, target_path: Path) -> None:
        ...
        source_syntax = Syntax(
            source_content, lexer, theme="monokai", line_numbers=True
        )
        target_syntax = Syntax(
            target_content, lexer, theme="monokai", line_numbers=True
        )
    ...
    def _display_file_content(self, file_path: Path, title: str) -> None:
    ...
    def display_summary(self, differences: List[Tuple[Path, Path, bool]]) -> None:
        ...
        for _source, target, has_diff in differences:
            if has_diff:
                status = (
                    "[red]Different[/red]"
                    if target.exists()
